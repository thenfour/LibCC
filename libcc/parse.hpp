...
namespace LibCC
{
	namespace Parse
	{
		...
		public:
			...
			PassthroughT(const Tchild& rhs, const wchar_t*& msg, bool isTrueMsg) :
				m_child(rhs),
				isTrue(isTrueMsg)
			{
				msgs.push_back(msg);
			...
			virtual bool Parse(ParseResult& result, ScriptReader& input)
			{
				...
				if(r && ((msgs.size() == 1 && isTrue) || (msgs.size() == 2)))
				{
					result.ParserMessage(msgs[0].c_str(), input.GetCursorCopy(), input);
					return true;
				}

				if(!r)
				{
					if(msgs.size() == 1 && !isTrue)
					{
						result.ParserMessage(msgs[0].c_str(), input.GetCursorCopy(), input);
						return false;
					}
					if(msgs.size() == 2)
					{
						result.ParserMessage(msgs[1].c_str(), input.GetCursorCopy(), input);
						return false;
					}
				}
				return r;
		...
		{
			return PassthroughT<Tchild>(child);
		}

		template<typename Tchild>
		inline PassthroughT<Tchild> FalseMsg(const std::wstring& msg, const Tchild& child)
		{
			return PassthroughT<Tchild>(child, msg, false);
		}

		template<typename Tchild>
		inline PassthroughT<Tchild> TrueMsg(const std::wstring& msg, const Tchild& child)
		{
			return PassthroughT<Tchild>(child, msg, true);
		}

		template<typename Tchild>
		inline PassthroughT<Tchild> FalseMsg(const wchar_t* msg, const Tchild& child)
		{
			return PassthroughT<Tchild>(child, msg, false);
		}

		template<typename Tchild>
		inline PassthroughT<Tchild> TrueMsg(const wchar_t* msg, const Tchild& child)
		{
			return PassthroughT<Tchild>(child, msg, true);
		}
