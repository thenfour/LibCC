/*
	THIS IS ADAPTED FROM REVISION 711 FROM JAN 21 2009
	OF STRINGUTIL.HPP FOR COMPARISON WITH THE LATEST VERSION.
*/
...
namespace LibCC_711
{
  ...
  template<typename InChar, typename OutChar>
	...
	inline HRESULT ToANSI(const wchar_t* in, size_t inLength, LibCC::Blob<BYTE>& out, UINT codepage = CP_ACP)
	{
		DWORD flags;
	...
	*/
	...
	{
		LibCC::Blob<BYTE> b;
		HRESULT hr = LibCC_711::ToANSI(in.c_str(), in.length(), b, tocodepage);
	...
	{
		LibCC::Blob<BYTE> b;
		HRESULT hr = LibCC_711::ToANSI(in, StringLength(in), b, tocodepage);
		if(FAILED(hr)) return hr;
		out.assign((const char*)b.GetBuffer(), b.Size());
		return hr;
	}
  ...
  inline std::basic_string<Char> StringReplace(const Char* src, const Char* searchString, const Char* replaceString)
  {
	...
	{
		LibCC::Blob<wchar_t> buf(s.length() + 1);
	...
	{
		LibCC::Blob<char> buf(s.length() + 1);
    ...
    */
    template<typename FloatType, typename _Char>
		inline void _RuntimeAppendFloat(const FloatType& _f, size_t Base, size_t DecimalWidthMax, size_t IntegralWidthMin, _Char PaddingChar, bool ForceSign, std::basic_string<_Char>& m_Composite)
		{
			...
			// normalized number.
			LibCC_711::_RuntimeAppendNormalizedFloat(_f, Base, DecimalWidthMax, IntegralWidthMin, PaddingChar, ForceSign, m_Composite);
		}
    ...
    inline void _AppendFloat(const FloatType& _f, std::basic_string<_Char>& m_Composite)
		{
	    return LibCC_711::_RuntimeAppendFloat<FloatType>(_f, Base, DecimalWidthMax, IntegralWidthMin, PaddingChar, ForceSign, m_Composite);
		}
...
namespace LibCC_711
{
  ...
  public:
    ...
    _This& d(double val)
		{
	    _AppendFloat<_Char, LibCC::DoublePrecisionFloat, Base, DecimalWidthMax, IntegralWidthMin, PaddingChar, ForceSign>(val, m_Composite);
			BuildCompositeChunk();
			return *this;
		}
    ...
    _This& d(double val, size_t DecimalWidthMax, size_t IntegralWidthMin = 1, _Char PaddingChar = '0', bool ForceSign = false, size_t Base = 10)
		{
			_RuntimeAppendFloat<LibCC::DoublePrecisionFloat, _Char>(val, Base, DecimalWidthMax, IntegralWidthMin, PaddingChar, ForceSign, m_Composite);
			BuildCompositeChunk();
			return *this;
		}
